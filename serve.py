...
if __name__ == '__main__':
    with Server(('', PORT), NoCacheHandler) as httpd:
        ...
        print()
        # Сервер намеренно виден всей сети — иначе телефон до него не достучится.
        # Но пароля здесь нет, и общая база открыта на чтение и на запись
        # любому, кто в той же сети. Дома это и есть задумка. В кафе, в отеле
        # и в гостевой сети — нет, и человек должен знать об этом сам,
        # а не выяснять постфактум.
        print('  Внимание: пока окно открыто, приложение и ваши данные доступны')
        print('  всем в этой сети без пароля. Дома это нормально, в чужой сети —')
        print('  закройте окно (Ctrl+C) и работайте через overmoney.html.')
        print()
        try:
            httpd.serve_forever()
