...
(function () {
  ...
  function personTargets(person) {
     ...
     *
     * Округление вниз, а не к ближайшему. С Math.round потолок мог оказаться
     * выше собственного правила: при 3266 ккал он давал 286 г, то есть
     * 35,03% калорий вместо обещанных 35%. Расхождение ничтожное — меньше
     * килокалории, — но правило, сформулированное как «не выше трети»,
     * должно выполняться буквально, иначе его нельзя проверить. */
    const pCap = Math.floor(kcal * PROTEIN_MAX_KCAL_SHARE / 4);
    if (p > pCap) p = pCap;
